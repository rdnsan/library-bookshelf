...
const addBook = (): void => {
  const uncompletedBookList = document.getElementById(
    Config.UNCOMPLETED_LIST_BOOK
  );
  const completedBookList = document.getElementById(Config.COMPLETED_LIST_BOOK);
  const inputBookTitle = document.getElementById(
    'bookTitle'
  ) as HTMLInputElement;
  const inputBookAuthor = document.getElementById(
    'bookAuthor'
  ) as HTMLInputElement;
...
const addBookToCompleted = (bookElement: any): void => {
  ...
  const bookAuthor = bookElement.firstChild.querySelector(
    'p:first-of-type > span'
  )?.innerText;
  const bookYear = bookElement.firstChild.querySelector(
    'p:last-of-type > span'
  )?.innerText;
...
export { addBook, addBookToCompleted, removeBook, undoBookFromCompleted };
